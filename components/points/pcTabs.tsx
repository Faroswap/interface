...
export default function PCTabs({
  ...
  return (
    <div className="md:flex hidden gap-5 text-secondary text-xl mb-3">
      <div
        className="flex items-center justify-end px-5 py-3 bg-paper rounded-2xl cursor-pointer"
        style={
          activeTab === Tab.Swap || activeTab === Tab.Total
            ? {
                background:
                  'linear-gradient(270deg, #326AFD 14.38%, #FEE94F 128.44%)',
                color: '#fff',
              }
            : {}
        }
      ...
      >
        <div
          className={clsx(
            'flex justify-center items-center w-10 h-10 rounded-lg',
            activeTab === Tab.Swap || activeTab === Tab.Total
              ? 'text-[#326AFD] bg-white'
              : 'bg-[#1A1A1B1A] text-[#1A1A1B80]',
          )}
        >
          <SwapIcon />
        </div>
